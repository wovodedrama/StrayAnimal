...
public class CodeGenerator {
    private static final String url = "jdbc:mysql://localhost:3306/stray-animal";
    private static final String username = "root";
    private static final String password = "123456";

    private static final String basePath = "D:\\毕设\\作品\\2022.3\\宠物领养\\springboot\\";

    public static void main(String[] args) throws Exception {
        String tableName = "activity";
        String modelName = "活动管理";
    ...
    private static void generate(String tableName) {
        FastAutoGenerator.create(url, username, password)
            .globalConfig(builder -> {
                builder.author("Kaze") // 设置作者
                    .enableSwagger()   // 开启 swagger 模式 (新版会自动识别 OpenAPI 3)
                    .disableOpenDir()  // 禁止打开输出目录
                    .outputDir(basePath + "src/main/java/"); // 指定输出目录
            })
            .packageConfig(builder -> {
                builder.parent("com.kaze.springboot") // 设置父包名
                    .moduleName(null) // 设置父包模块名
                    .pathInfo(Collections.singletonMap(OutputFile.xml, basePath + "src/main/resources/mapper/")); // 注意 key 变成了 OutputFile.xml
            })
            .strategyConfig(builder -> {
                builder.addInclude(tableName) // 设置需要生成的表名
                    ...
                    .entityBuilder()
                    .enableLombok()
                    .enableFileOverride() // 旧版在 globalConfig，新版在具体的 builder 里
